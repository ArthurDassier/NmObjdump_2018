...
{
    if (compare32(sym, shdr, getter(NULL, 42), ".debug") && c == 0)
        c = 'N';
    else if (ELF32_ST_BIND(sym.st_info) == STB_GNU_UNIQUE && c == 0)
        c = 'u';
...
{
    if ((compare32(sym, shdr, getter(NULL, 42), ".fini_array")
    || ELF32_ST_TYPE(sym.st_info) == STT_FUNC) && c == 0)
        c = 'T';
    else if (shdr[sym.st_shndx].sh_type == SHT_PROGBITS
    && shdr[sym.st_shndx].sh_flags == SHF_ALLOC && c == 0)
        c = 'R';
    else if ((compare32(sym, shdr, getter(NULL, 42), ".eh_frame_hdr")
    || ELF32_ST_TYPE(sym.st_info) == STT_FUNC) && c == 0)
        c = 'R';
    else if ((ELF32_ST_TYPE(sym.st_info) == STT_OBJECT
    || ELF32_ST_TYPE(sym.st_info) == STT_NOTYPE
    || compare32(sym, shdr, getter(NULL, 42), ".data")
    || compare32(sym, shdr, getter(NULL, 42), ".data1")) && c == 0)
        c = 'D';
    ...
    && shdr[sym.st_shndx].sh_flags == (SHF_ALLOC | SHF_WRITE) && c == 0)
        c = 'D';
    else if (compare32(sym, shdr, getter(NULL, 42), ".rodata") && c == 0)
        c = 'R';
...
char found_type32(Elf32_Sym sym, Elf32_Shdr *shdr)
{
    ...
    if (compare32(sym, shdr, getter(NULL, 42), ".bss"))
        c = 'B';
    else if (ELF32_ST_BIND(sym.st_info) == STB_WEAK) {
        c = 'W';
        if (sym.st_shndx == SHN_UNDEF)
    ...
    && shdr[sym.st_shndx].sh_flags == (SHF_ALLOC | SHF_EXECINSTR))
        c = 'T';
    else if (compare32(sym, shdr, getter(NULL, 42), ".text")
    || ELF32_ST_TYPE(sym.st_info) == STT_FUNC)
        c = 'T';
    else if (compare32(sym, shdr, getter(NULL, 42), ".init_array")
    || ELF32_ST_TYPE(sym.st_info) == STT_FUNC)
        c = 'T';
